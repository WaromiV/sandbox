...
export function agentRoutes(
  ...
  options: {
    ...
    /**
     * When set, the routes hide paperclip-native agent rows from
     * listings and reject create/update/delete on rows that aren't
     * mirrored from openclaw. The bridge owns identity in that mode.
     */
    openclawBridgeEnabled?: boolean;
  } = {},
) {
  const openclawBridgeEnabled = options.openclawBridgeEnabled === true;

  /**
   * In bridge mode every UI-driven agent mutation must go through openclaw.
   * Use on routes that modify agent identity (PATCH, DELETE) — operational
   * routes (pause, resume, terminate, keys) remain available so other parts
   * of paperclip can still orchestrate runs.
   */
  function refuseAgentMutationsForBridgeMode(res: import("express").Response): boolean {
    if (!openclawBridgeEnabled) return false;
    res.status(409).json({
      error: "openclaw_bridge_enabled",
      message:
        "Paperclip is in openclaw-bridge mode. Agents are managed by openclaw — change them there.",
    });
    return true;
  }
  ...
  router.get("/companies/:companyId/agents", async (req, res) => {
    ...
    // Bridge mode: paperclip-native agents (CRUD-created) are invisible.
    // Only openclaw-mirrored rows count as "agents" in the UI.
    const filtered = openclawBridgeEnabled
      ? result.filter(
          (a) =>
            (a as unknown as { externalSource?: string }).externalSource === "openclaw",
        )
      : result;
  ...
  router.post("/companies/:companyId/agents", validate(createAgentSchema), async (req, res) => {
    if (openclawBridgeEnabled) {
      res.status(409).json({
        error: "openclaw_bridge_enabled",
        message:
          "Paperclip is in openclaw-bridge mode. Agents are managed by openclaw — create or edit them there.",
      });
      return;
    }
    const companyId = req.params.companyId as string;
  ...
  router.patch("/agents/:id", validate(updateAgentSchema), async (req, res) => {
    if (refuseAgentMutationsForBridgeMode(res)) return;
    const id = req.params.id as string;
    const existing = await svc.getById(id);
    if (!existing) {
      res.status(404).json({ error: "Agent not found" });
