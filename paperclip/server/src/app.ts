...
export async function createApp(
  ...
  // Load openclaw bridge config early so downstream route mounts can flip
  // into "bridge mode" (hide UI affordances for manual agent CRUD).
  const openclawConfig = loadOpenclawBridgeConfig();
  ...
  // OpenClaw bridge: paperclip's agents come from openclaw via `agents.list`.
  ...
  if (openclawConfig) {
    ...
    void openclawBridge.start().catch((err) => {
      logger.error({ err }, "openclaw bridge failed to start");
    });
  } else {
    logger.info(
