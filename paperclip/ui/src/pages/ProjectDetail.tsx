...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onSuccess: (updatedProject, archived) => {
      ...
      if (archived) {
        pushToast({ title: `"${name}" ${t("has been archived")}`, tone: "success" });
        navigate("/dashboard");
      } else {
        pushToast({ title: `"${name}" ${t("has been unarchived")}`, tone: "success" });
      }
    },
    onError: (_, archived) => {
      pushToast({
        ...
        <div className="min-w-0 space-y-2">
          ...
          {project.managedByPlugin ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-muted px-3 py-1 text-[11px] font-medium text-muted-foreground">
              ...
              {t("Managed by")} {project.managedByPlugin.pluginDisplayName}
            </div>
          ) : null}
        </div>
      </div>
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            ...
            ...(project.managedByPlugin ? [{ value: "plugin-operations", label: t("Plugin operations") }] : []),
