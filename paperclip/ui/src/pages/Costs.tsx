    ...
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={`${eventCount} ${eventCount === 1 ? t("total event in range") : t("total events in range")}`}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("Credits")}
      ...
      <div className="space-y-5">
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={`${formatTokens(inferenceTokenTotal)} ${t("tokens across request-scoped events")}`}
              icon={DollarSign}
            />
            <MetricTile
              label={t("Budget")}
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `${budgetData?.pausedAgentCount ?? 0} ${t("agents paused")} · ${budgetData?.pausedProjectCount ?? 0} ${t("projects paused")}`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? `${formatCents(spendData.summary.spendCents)} ${t("of")} ${formatCents(spendData.summary.budgetCents)}`
            ...
            <MetricTile
              ...
              subtitle={`${formatCents(financeData?.summary.debitCents ?? 0)} ${t("debits")} · ${formatCents(financeData?.summary.creditCents ?? 0)} ${t("credits")}`}
              icon={ReceiptText}
            />
            <MetricTile
              label={t("Finance events")}
              value={String(financeData?.summary.eventCount ?? 0)}
              subtitle={`${formatCents(financeData?.summary.estimatedDebitCents ?? 0)} ${t("estimated in range")}`}
              icon={ArrowUpRight}
            />
          </div>
      </div>
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? `${t("Budget")} ${formatCents(spendData.summary.budgetCents)}`
                            : t("Unlimited budget")}
                        </div>
                      </div>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {spendData.summary.utilizationPercent}% {t("of monthly budget consumed in this range.")}
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
