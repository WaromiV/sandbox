...
const concurrencyPolicyDescriptions: Record<string, string> = {
  coalesce_if_active: t("If a run is already active, keep just one follow-up run queued."),
  always_enqueue: t("Queue every trigger occurrence, even if the routine is already running."),
  skip_if_active: t("Drop new trigger occurrences while a run is still active."),
};
const catchUpPolicyDescriptions: Record<string, string> = {
  skip_missed: t("Ignore windows that were missed while the scheduler or routine was paused."),
  enqueue_missed_with_cap: t("Catch up missed schedule windows in capped batches after recovery."),
};
...
export function buildRoutineGroups(
  ...
  if (groupByValue === "project") {
    ...
    return Object.keys(groups)
      .sort((left, right) => {
        const leftLabel = left === "__no_project" ? t("No project") : (projectById.get(left)?.name ?? t("Unknown project"));
        const rightLabel = right === "__no_project" ? t("No project") : (projectById.get(right)?.name ?? t("Unknown project"));
        return leftLabel.localeCompare(rightLabel);
      })
      .map((key) => ({
        key,
        label: key === "__no_project" ? t("No project") : (projectById.get(key)?.name ?? t("Unknown project")),
        items: groups[key]!,
      }));
  }
  ...
  return Object.keys(groups)
    .sort((left, right) => {
      const leftLabel = left === "__unassigned" ? t("Unassigned") : (agentById.get(left)?.name ?? t("Unknown agent"));
      const rightLabel = right === "__unassigned" ? t("Unassigned") : (agentById.get(right)?.name ?? t("Unknown agent"));
      return leftLabel.localeCompare(rightLabel);
    })
    .map((key) => ({
      key,
      label: key === "__unassigned" ? t("Unassigned") : (agentById.get(key)?.name ?? t("Unknown agent")),
      items: groups[key]!,
    }));
}
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("Paperclip could not update the routine."),
        tone: "error",
      });
    },
  });
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        ...
        <TabsContent value="routines" className="space-y-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-sm text-muted-foreground">
              {(routines ?? []).length} {(routines ?? []).length === 1 ? t("routine") : t("routines")}
            </p>
