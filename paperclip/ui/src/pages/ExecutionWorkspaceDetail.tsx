...
function parseWorkspaceRuntimeJson(value: string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: t("Workspace commands JSON must be a JSON object."),
      };
    }
...
function validateForm(form: WorkspaceFormState) {
  ...
  if (repoUrl) {
    try {
      ...
      return t("Repo URL must be a valid URL.");
    }
  }
    ...
    <div className="flex flex-col gap-3 border-b border-border px-3 py-3 last:border-b-0 sm:flex-row sm:items-center">
      <div className="min-w-0 flex-1 space-y-1.5">
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span>{routine.assigneeAgentId ? t("Default agent set") : t("Choose agent when running")}</span>
          <span>{t("Last run")} {formatOptionalDateTime(routine.lastRun?.triggeredAt ?? routine.lastTriggeredAt)}</span>
...
function ExecutionWorkspaceRoutinesList({
  ...
  const runRoutine = useMutation({
    ...
    onSuccess: async (_, { id }) => {
      ...
      pushToast({
        title: t("Routine started"),
        body: t("Paperclip created a run using this execution workspace."),
        tone: "success",
      });
    },
    onSettled: () => {
    ...
    onError: (mutationError) => {
      pushToast({
        ...
        body: mutationError instanceof Error ? mutationError.message : t("Paperclip could not start the routine run."),
        tone: "error",
      });
    },
  });
  ...
  return (
    <>
      <Card className="rounded-none">
        <CardHeader>
          <CardTitle>{t("Workspace routines")}</CardTitle>
          <CardDescription>
            {t("Routines that use workspace-specific variables can be run against this execution workspace.")}
          </CardDescription>
        </CardHeader>
        <CardContent>
          {isLoading ? (
            <p className="text-sm text-muted-foreground">{t("Loading routines...")}</p>
          ) : error ? (
            ...
            <div className="flex flex-col items-center gap-2 py-10 text-center">
              ...
              <p className="text-sm text-muted-foreground">
                {t("No routines use workspace-specific variables yet.")}
              </p>
            </div>
          ) : (
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, request) => {
      ...
      setRuntimeActionMessage(
        request.action === "run"
          ? t("Workspace job completed.")
          : request.action === "stop"
            ? t("Workspace service stopped.")
            : request.action === "restart"
              ? t("Workspace service restarted.")
              : t("Workspace service started."),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setRuntimeActionErrorMessage(error instanceof Error ? error.message : t("Failed to control workspace commands."));
    },
  });
    ...
    <>
      <div className="space-y-4 overflow-hidden sm:space-y-6">
        ...
        <Tabs value={activeTab ?? "issues"} onValueChange={(value) => handleTabChange(value as ExecutionWorkspaceTab)}>
          <PageTabBar
            items={[
              ...
              { value: "runtime_logs", label: t("Runtime logs") },
        ...
        {activeTab === "services" ? (
          <WorkspaceRuntimeControls
            ...
            serviceEmptyMessage={
              effectiveRuntimeConfig
                ? t("No services have been started for this execution workspace yet.")
                : t("No workspace command config is defined for this execution workspace yet.")
            }
            jobEmptyMessage={t("No one-shot jobs are configured for this execution workspace yet.")}
            disabledHint={
              canStartRuntimeServices
                ? null
                : t("Execution workspaces need a working directory before local commands can run, and services also need runtime config.")
            }
            onAction={(request) => controlRuntimeServices.mutate(request)}
          />
          ...
          <div className="space-y-4 sm:space-y-6">
            <Card className="rounded-none">
              <CardHeader>
                ...
                <CardDescription>
                  {t("Edit the concrete path, repo, branch, provisioning, teardown, and runtime overrides attached to this execution workspace.")}
                </CardDescription>
                <CardAction>
                  <Button
                    variant="destructive"
              ...
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("Source control")}</div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("Lifecycle commands")}</div>
                ...
                <div className="space-y-4">
                  <div className="text-xs font-medium uppercase tracking-widest text-muted-foreground">{t("Runtime config")}</div>
                  ...
                  <details className="rounded-md border border-dashed border-border/70 bg-background px-4 py-3">
                    <summary className="cursor-pointer text-sm font-medium">{t("Advanced runtime JSON")}</summary>
                    <p className="mt-2 text-sm text-muted-foreground">
                      {t("Override the inherited workspace command model only when this execution workspace truly needs different service or job behavior.")}
                    </p>
                    <div className="mt-3">
                      <Field label={t("Workspace commands JSON")} hint={t("Legacy `services` arrays still work, but `commands` supports both services and jobs.")}>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("Workspace context")}</CardTitle>
                <CardDescription>{t("Linked objects and relationships")}</CardDescription>
              </CardHeader>
              <CardContent>
            ...
            <Card className="rounded-none">
              <CardHeader>
                <CardTitle>{t("Concrete location")}</CardTitle>
                <CardDescription>{t("Paths and refs")}</CardDescription>
              </CardHeader>
              <CardContent>
          ...
          <Card className="rounded-none">
            <CardHeader>
              <CardTitle>{t("Runtime and cleanup logs")}</CardTitle>
              <CardDescription>{t("Recent operations")}</CardDescription>
            </CardHeader>
            <CardContent>
