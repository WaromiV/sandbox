...
const permissionLabels: Record<PermissionKey, string> = {
  "agents:create": t("Create agents"),
  "users:invite": t("Invite humans and agents"),
  "users:manage_permissions": t("Manage members and grants"),
  "tasks:assign": t("Assign tasks"),
  "tasks:assign_scope": t("Assign scoped tasks"),
  "tasks:manage_active_checkouts": t("Manage active task checkouts"),
  "joins:approve": t("Approve join requests"),
  "environments:manage": t("Manage environments"),
};
      ...
      pushToast({
        ...
        body:
          result.reassignedIssueCount > 0
            ? `${result.reassignedIssueCount} ${result.reassignedIssueCount === 1 ? t("assigned issue cleaned up.") : t("assigned issues cleaned up.")}`
            : undefined,
        tone: "success",
      });
    },
    ...
    <div className="max-w-6xl space-y-8">
      <div className="space-y-3">
        ...
        <p className="max-w-3xl text-sm text-muted-foreground">
          {t("Manage company user memberships, membership status, and explicit permission grants for")} {selectedCompany?.name}.
        </p>
      </div>
        ...
        {access?.canApproveJoinRequests && pendingHumanJoinRequests.length > 0 ? (
          <div className="space-y-3 rounded-xl border border-border px-4 py-4">
            <div className="flex flex-wrap items-center justify-between gap-2">
              ...
              <Badge variant="outline">{pendingHumanJoinRequests.length} {t("pending")}</Badge>
            </div>
            <div className="space-y-3">
              {pendingHumanJoinRequests.map((request) => (
                <PendingJoinRequestCard
                  ...
                  detail={`${t("Submitted")} ${new Date(request.createdAt).toLocaleString()}`}
                ...
                >
                  ...
                  <div className="text-sm">
                    {member.membershipRole
                      ? t(HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[member.membershipRole])
                      : t("Unset")}
                  </div>
                  <div>
      ...
      <Dialog open={!!editingMember} onOpenChange={(open) => !open && setEditingMemberId(null)}>
        <DialogContent className="max-w-2xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("Update company role, membership status, and explicit grants for")} {editingMember?.user?.name || editingMember?.user?.email || editingMember?.principalId}.
            </DialogDescription>
          </DialogHeader>
          {editingMember && (
              ...
              <div className="space-y-3">
                ...
                <div className="rounded-lg border border-border px-3 py-3">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {draftRole
                      ? `${t(HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole])} ${t("currently includes these permissions automatically.")}`
                      ...
                      <span className="space-y-1">
                        ...
                        {implicitGrantSet.has(permissionKey) ? (
                          <span className="block text-xs text-muted-foreground">
                            {t("Included implicitly by the")} {draftRole ? t(HUMAN_COMPANY_MEMBERSHIP_ROLE_LABELS[draftRole]) : t("selected")} {t("role. Add an explicit grant only if it should stay after the role changes.")}
                          </span>
                        ) : null}
      ...
      <Dialog open={!!removingMember} onOpenChange={(open) => !open && setRemovingMemberId(null)}>
        <DialogContent className="max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription>
              {t("Archive")} {memberDisplayName(removingMember)} {t("and move active assignments before hiding this user from assignment fields.")}
            </DialogDescription>
          </DialogHeader>
          {removingMember && (
            <div className="space-y-5">
              <div className="rounded-lg border border-border px-3 py-3">
                ...
                <div className="mt-2 text-sm text-muted-foreground">
                  {assignedIssuesQuery.isLoading
                    ...
                    : `${assignedIssues.length} ${assignedIssues.length === 1 ? t("open assigned issue") : t("open assigned issues")}`}
                </div>
              </div>
                  ...
                  <div className="max-h-36 overflow-auto rounded-lg border border-border">
                    ...
                    {assignedIssues.length > 6 ? (
                      <div className="px-3 py-2 text-sm text-muted-foreground">
                        {assignedIssues.length - 6} {assignedIssues.length - 6 === 1 ? t("more issue") : t("more issues")}
                      </div>
                    ) : null}
                  </div>
                </div>
