...
function describeScope(scope: CompanySearchScope) {
  if (scope === "all") return t("All scopes");
...
function shapeError(error: unknown): { message: string; status?: number } {
  if (!error) return { message: t("Unknown error") };
      ...
      <div className="border-b border-border px-4 py-3 sm:px-6">
        ...
        <div className="relative">
          ...
          <Input
            ...
            placeholder={t("Search issues, comments, documents, agents, projects…")}
  ...
  if (showInitialState) {
    return (
      <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 px-4 py-10 sm:px-6">
        <div>
          <h2 className="text-lg font-semibold">{t("Type to search company memory.")}</h2>
          <p className="mt-1 text-sm text-muted-foreground">
            {t("Issues, comments, plan documents, agents, projects — same surface, ranked by relevance.")}
          </p>
        </div>
        ...
        <ul className="space-y-1 text-xs text-muted-foreground">
          <li>
            <span className="font-medium text-foreground">{t("Identifier lookup:")}</span> {t("type")}{" "}
            <code className="rounded bg-muted px-1 py-0.5 text-[11px]">PAP-123</code> {t("to jump straight to an issue.")}
          </li>
          <li>
            <span className="font-medium text-foreground">{t("Quoted phrases:")}</span>{" "}
            {t("wrap a phrase in quotes to match the exact sequence.")}
          </li>
          <li>
            <span className="font-medium text-foreground">⌘K:</span>{" "}
            {t("reopens the command palette pre-seeded with your current query.")}
          </li>
        </ul>
      </div>
    );
  ...
  if (hasError) {
    ...
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">{t("Couldn’t run that search")}</div>
        <p className="text-sm text-muted-foreground">
          {status ? `${t("The server returned")} ${status}.` : t("The request failed.")}{" "}
          {t("Your input and filters are still here, so you can retry or fall back to the Issues filter.")}
        </p>
    ...
    return (
      <div className="flex flex-col gap-2 px-2 py-3 sm:px-4">
        <div className="px-3 text-xs text-muted-foreground" data-testid="search-loading">
          {t("Searching for")} &ldquo;{trimmedQuery}&rdquo;…
        </div>
  ...
  if (isEmpty) {
    return (
      <div className="mx-auto flex w-full max-w-xl flex-col items-center justify-center gap-3 px-4 py-12 text-center">
        ...
        <div className="text-base font-semibold">{t("No results for")} &ldquo;{trimmedQuery}&rdquo;</div>
        <p className="text-sm text-muted-foreground">
          {t("We couldn’t find a match in")} {describeScope(scope).toLowerCase()}.{" "}
          {t("Try widening the scope or rephrasing your query.")}
        </p>
        <div className="flex flex-wrap items-center justify-center gap-2">
          {scope !== "all" ? (
          ...
          <Button onClick={openNewIssue} size="sm" variant="default">
            ...
            {t("Create issue from this query")}
          </Button>
        ...
        <ul className="mt-2 space-y-0.5 text-xs text-muted-foreground">
          <li>{t("Try fewer tokens or a single distinctive term.")}</li>
          <li>
            {t("Use an identifier shortcut like")} <code className="rounded bg-muted px-1 py-0.5">PAP-123</code>.
          </li>
          <li>{t("Wrap multi-word phrases in quotes.")}</li>
        </ul>
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full max-w-[960px] flex-col px-2 sm:px-4" data-testid="search-results">
      <div className="flex items-center justify-between py-2 text-[11px] uppercase tracking-wide text-muted-foreground">
        <span>
          {totalResults === 1 ? t("1 result") : `${totalResults} ${t("results")}`} · {t("sorted by relevance")}
        </span>
        {isFetching ? <span aria-live="polite" className="normal-case tracking-normal">{t("Updating…")}</span> : null}
      </div>
