  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("Unknown error");
        throw new Error(
          failures.length === 1
            ? `${t("Failed to disable 1 timer heartbeat:")} ${detail}`
            : `${t("Failed to disable")} ${failures.length} ${t("of")} ${enabled.length} ${t("timer heartbeats. First error:")} ${detail}`,
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("Failed to disable all heartbeats."));
    },
  });
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        ...
        {anyEnabled && (
          <Button
            ...
            onClick={() => {
              const noun = enabledCount === 1 ? t("enabled agent") : t("enabled agents");
              if (!window.confirm(`${t("Disable timer heartbeats for all")} ${enabledCount} ${noun}?`)) {
                return;
              }
