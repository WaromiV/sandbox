...
export function Agents() {
  ...
  // When the openclaw bridge owns agent identity, paperclip hides its own
  // create/edit/delete affordances — operators manage agents in openclaw.
  const { data: openclawStatus } = useQuery({
    queryKey: ["openclaw", "status"],
    queryFn: () => openclawApi.status(),
    staleTime: 30_000,
  });
  const openclawBridgeEnabled = openclawStatus?.enabled === true;
  ...
  return (
    <div className="space-y-4">
      ...
      {filtered.length > 0 && (
        <p className="text-xs text-muted-foreground">{t("Agents:")} {filtered.length}</p>
      )}

      {error && <p className="text-sm text-destructive">{error.message}</p>}

      {agents && agents.length === 0 && (
        openclawBridgeEnabled ? (
          <EmptyState
            icon={Bot}
            message={t("Agents are managed by openclaw. Define them in your openclaw config; they will appear here automatically.")}
          />
        ) : (
          <EmptyState
            icon={Bot}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <span className="text-xs text-muted-foreground ml-2">
            {t(roleLabels[node.role] ?? node.role)}
