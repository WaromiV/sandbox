...
export function JoinRequestQueue() {
  ...
  const approveMutation = useMutation({
    ...
    onSuccess: async () => {
      ...
      pushToast({ title: t("Join request approved"), tone: "success" });
    },
  });
  ...
  const rejectMutation = useMutation({
    ...
    onSuccess: async () => {
      await queryClient.invalidateQueries({ queryKey: queryKeys.access.joinRequests(selectedCompanyId!, `${status}:${requestType}`) });
      pushToast({ title: t("Join request rejected"), tone: "success" });
    },
  });

  if (!selectedCompanyId) {
    return <div className="text-sm text-muted-foreground">{t("Select a company to review join requests.")}</div>;
  }

  if (requestsQuery.isLoading) {
    return <div className="text-sm text-muted-foreground">{t("Loading join requests…")}</div>;
  }
    ...
    <div className="max-w-6xl space-y-6">
      <div className="space-y-3">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold">{t("Join Request Queue")}</h1>
        </div>
            ...
            <div key={request.id} className="rounded-xl border border-border bg-card p-4">
                  ...
                  <div>
                    <div className="text-base font-medium">
                      {request.requestType === "human"
                        ? request.requesterUser?.name || request.requestEmailSnapshot || request.requestingUserId || t("Unknown human requester")
                        : request.agentName || t("Unknown agent requester")}
                    </div>
              ...
              <div className="mt-4 grid gap-3 text-sm text-muted-foreground md:grid-cols-2">
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  <div className="text-xs font-medium uppercase tracking-wide">{t("Invite context")}</div>
                  <div className="mt-2">
                    {request.invite
                      ? `${request.invite.allowedJoinTypes} ${t("join invite")}${request.invite.humanRole ? ` • ${t("default role")} ${request.invite.humanRole}` : ""}`
                ...
                <div className="rounded-lg border border-border bg-background px-3 py-2">
                  <div className="text-xs font-medium uppercase tracking-wide">{t("Request details")}</div>
                  <div className="mt-2">{t("Submitted")} {new Date(request.createdAt).toLocaleString()}</div>
                  <div>{t("Source IP")} {request.requestIp}</div>
