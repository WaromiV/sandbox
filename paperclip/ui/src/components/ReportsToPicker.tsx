...
export function ReportsToPicker({
  ...
  disabled = false,
  excludeAgentIds = [],
  disabledEmptyLabel = t("Reports to: N/A (CEO)"),
  chooseLabel = t("Reports to..."),
}: {
  agents: Agent[];
  value: string | null;
        ...
        >
          {unknownManager ? (
            <>
              ...
              <span className="min-w-0 truncate text-muted-foreground">{t("Unknown manager (stale ID)")}</span>
            </>
          ) : current ? (
            <>
              ...
              >
                {`${t("Reports to")} ${current.name}${terminatedManager ? ` (${t("terminated")})` : ""}`}
              </span>
            </>
          ) : (
            <>
      ...
      <PopoverContent className="w-48 p-1" align="start">
        ...
        {terminatedManager && (
          <div className="flex min-w-0 items-center gap-2 overflow-hidden px-2 py-1.5 text-xs text-muted-foreground border-b border-border mb-0.5">
            ...
            <span className="min-w-0 truncate">
              {t("Current:")} {current.name} ({t("terminated")})
            </span>
          </div>
        )}
        {unknownManager && (
