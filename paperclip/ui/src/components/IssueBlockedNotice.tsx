...
function BlockerRecoveryIndicator({ action }: { action: IssueRecoveryAction }) {
  ...
  return (
    <span
      ...
      title={`${t(tone.label)} — ${t("open the source issue to act.")}`}
...
export function IssueBlockedNotice({
  ...
  const blockerLabel = blockers.length === 1 ? t("the linked issue") : t("the linked issues");
      ...
      <div className="flex items-start gap-2">
        ...
        <div className="min-w-0 space-y-1.5">
          {showSuccessfulRunHandoff ? (
            <>
              <p className="font-medium leading-5">{t("This issue still needs a next step.")}</p>
              <p className="leading-5">
                {t("A run finished successfully, but this issue is still open in")}{" "}
                <code className="rounded bg-amber-100 px-1 py-0.5 text-[12px] dark:bg-amber-400/15">
                  in_progress
                </code>{" "}
                {t("with no clear owner for the next action.")}
              </p>
              <ul className="list-disc space-y-1 pl-5 text-xs leading-5 text-amber-900 dark:text-amber-100">
                <li>{t("Mark it done or cancelled.")}</li>
                <li>{t("Send it for review or ask for input.")}</li>
                <li>{t("Mark it blocked with a blocker owner.")}</li>
                <li>{t("Delegate follow-up work or queue a continuation.")}</li>
              </ul>
              <div className="flex flex-wrap gap-1.5 text-xs">
                ...
                <span className="rounded-md border border-amber-300/70 bg-background/80 px-2 py-1 text-amber-900 dark:border-amber-500/40 dark:bg-background/40 dark:text-amber-100">
                  {t("Corrective wake queued for")} {agentName ?? t("the assignee")}
                </span>
              </div>
              {successfulRunHandoff.detectedProgressSummary ? (
                <p className="text-xs leading-5 text-amber-800 dark:text-amber-200">
                  {t("Detected progress:")} {successfulRunHandoff.detectedProgressSummary}
                </p>
              ) : null}
            </>
          ) : null}
                ...
                {blockers.length > 0
                  ? isStalled
                    ? stalledLeafBlockers.length > 1
                      ? <>{t("Work on this issue is blocked by")} {blockerLabel}{t(", but the chain is stalled in review without a clear next step. Resolve the stalled reviews below or remove them as blockers.")}</>
                      : <>{t("Work on this issue is blocked by")} {blockerLabel}{t(", but the chain is stalled in review without a clear next step. Resolve the stalled review below or remove it as a blocker.")}</>
                    : <>{t("Work on this issue is blocked by")} {blockerLabel} {blockers.length === 1 ? t("until it is complete.") : t("until they are complete.")} {t("Comments still wake the assignee for questions or triage.")}</>
                  : <>{t("Work on this issue is blocked until it is moved back to todo.")} {t("Comments still wake the assignee for questions or triage.")}</>}
              </p>
              {blockers.length > 0 ? (
