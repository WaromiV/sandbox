    ...
    <div className="mb-3 rounded-lg border border-border bg-muted/30 px-3 py-2">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        <div className="min-w-0">
          <div className="text-sm font-medium text-foreground">{t("Monitor scheduled")}</div>
          <div className="text-xs text-muted-foreground">
            {t("Next check")} {formatDateTime(monitor.nextCheckAt)} ({formatMonitorOffset(monitor.nextCheckAt)})
          </div>
          {monitor.notes ? (
          ...
          {monitor.attemptCount > 0 ? (
            <div className="mt-1 text-xs text-muted-foreground">{t("Attempt")} {monitor.attemptCount}</div>
          ) : null}
        </div>
        {onCheckNow ? (
          <Button
          ...
          >
            {checkingNow ? t("Checking...") : t("Check now")}
          </Button>
        ) : null}
      </div>
    </div>
