...
const LIVENESS_COPY: Record<RunLivenessState, LivenessCopy> = {
  completed: {
    ...
    description: t("Issue reached a terminal state."),
  },
  advanced: {
    label: t("Advanced"),
    tone: "border-cyan-500/30 bg-cyan-500/10 text-cyan-700 dark:text-cyan-300",
    description: t("Run produced concrete evidence of progress."),
  },
  plan_only: {
    label: t("Plan only"),
    tone: "border-amber-500/30 bg-amber-500/10 text-amber-700 dark:text-amber-300",
    description: t("Run described future work without concrete action evidence."),
  },
  empty_response: {
    label: t("Empty response"),
    tone: "border-orange-500/30 bg-orange-500/10 text-orange-700 dark:text-orange-300",
    description: t("Run finished without useful output."),
  },
  blocked: {
    label: t("Blocked"),
    tone: "border-yellow-500/30 bg-yellow-500/10 text-yellow-700 dark:text-yellow-300",
    description: t("Run or issue declared a blocker."),
  },
  failed: {
    label: t("Failed"),
    tone: "border-red-500/30 bg-red-500/10 text-red-700 dark:text-red-300",
    description: t("Run ended unsuccessfully."),
  },
  needs_followup: {
    label: t("Needs follow-up"),
    tone: "border-sky-500/30 bg-sky-500/10 text-sky-700 dark:text-sky-300",
    description: t("Run produced useful output but did not prove concrete progress."),
  },
};

const PENDING_LIVENESS_COPY: LivenessCopy = {
  label: t("Checks after finish"),
  tone: "border-border bg-background text-muted-foreground",
  description: t("Liveness is evaluated after the run finishes."),
};

const RETRY_PENDING_LIVENESS_COPY: LivenessCopy = {
  label: t("Retry pending"),
  tone: "border-cyan-500/30 bg-cyan-500/10 text-cyan-700 dark:text-cyan-300",
  description: t("Paperclip queued an automatic retry that has not started yet."),
};

const MISSING_LIVENESS_COPY: LivenessCopy = {
  label: t("No liveness data"),
  tone: "border-border bg-background text-muted-foreground",
  description: t("This run has no persisted liveness classification."),
};
...
function modelProfileTitle(summary: ModelProfileSummary) {
  const lines = [`${t("Requested:")} ${summary.requested}`];
  if (summary.applied) lines.push(`${t("Applied:")} ${summary.applied}`);
  if (summary.configSource) lines.push(`${t("Source:")} ${summary.configSource}`);
  if (summary.fallbackReason) lines.push(`${t("Fallback:")} ${summary.fallbackReason}`);
  return lines.join("\n");
}
...
function runSummary(run: LedgerRun, agentMap: ReadonlyMap<string, Pick<Agent, "name">>) {
  ...
  if (run.status === "running") return `${t("Running now by")} ${agentName}`;
  if (run.status === "queued") return `${t("Queued for")} ${agentName}`;
  if (run.status === "scheduled_retry") return `${t("Automatic retry scheduled for")} ${agentName}`;
  return `${statusLabel(run.status)} ${t("by")} ${agentName}`;
}
...
function stopReasonLabel(run: RunForIssue) {
  ...
  if (timeoutFired || stopReason === "timeout") {
    return timeoutText ? `${t("timeout")} (${timeoutText})` : t("timeout");
  }
  if (stopReason === "max_turns_exhausted" || stopReason === "turn_limit_exhausted") return t("max turns exhausted");
  if (stopReason === "budget_paused") return t("budget paused");
  if (stopReason === "cancelled") return t("cancelled");
  if (stopReason === "paused") return t("paused by board");
  if (stopReason === "process_lost") return t("process lost");
  if (stopReason === "adapter_failed") return t("adapter failed");
  if (stopReason === "completed") return timeoutText ? `${t("completed")} (${timeoutText})` : t("completed");
  return timeoutText;
}

function stopStatusLabel(run: LedgerRun, stopReason: string | null) {
  if (stopReason) return stopReason;
  if (run.status === "scheduled_retry") return t("Retry pending");
  if (run.status === "queued") return t("Waiting to start");
  if (run.status === "running") return t("Still running");
  if (!run.livenessState) return t("Unavailable");
  return t("No stop reason");
}

function lastUsefulActionLabel(run: LedgerRun) {
  if (run.status === "scheduled_retry") return t("Waiting for next attempt");
  if (run.lastUsefulActionAt) return relativeTime(run.lastUsefulActionAt);
  if (isActiveRun(run)) return t("No action recorded yet");
  if (run.livenessState === "plan_only" || run.livenessState === "needs_followup") {
    return t("No concrete action");
  }
  if (run.livenessState === "empty_response") return t("No useful output");
  if (!run.livenessState) return t("Unavailable");
  return t("None recorded");
}

function continuationLabel(run: LedgerRun) {
  if (!run.continuationAttempt || run.continuationAttempt <= 0) return null;
  return `${t("Continuation attempt")} ${run.continuationAttempt}`;
}
...
function formatSilenceAge(ms: number | null | undefined) {
  ...
  if (totalMinutes < 1) return t("under 1 minute");
...
function watchdogDecisionErrorMessage(error: unknown) {
  if (error instanceof ApiError && error.status === 403) {
    return t("Only the board or the assigned recovery owner can record watchdog decisions");
  }
  return error instanceof Error && error.message.trim().length > 0
    ? error.message
    : t("Paperclip could not record the watchdog decision.");
}
...
export function IssueRunLedger({
  ...
  const watchdogDecision = useMutation({
    ...
    onError: (error) => {
      ...
      pushToast({
        title: t("Watchdog decision not recorded"),
        body: message,
        tone: "error",
...
export function IssueRunLedgerContent({
  ...
  return (
    <section className="space-y-3" aria-label={t("Issue run ledger")}>
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <h3 className="text-sm font-medium text-muted-foreground">{t("Run ledger")}</h3>
          <p className="text-xs text-muted-foreground">
            {latestRun
              ? runSummary(latestRun, agentMap)
              : issueStatus === "in_progress"
                ? t("Waiting for the first run record.")
                : t("No runs linked yet.")}
          </p>
        </div>
        {latestRun ? (
          <Link
      ...
      {children.total > 0 ? (
        <div className="rounded-md border border-border/70 px-3 py-2">
          <div className="flex flex-wrap items-center gap-2 text-xs">
            <span className="font-medium text-foreground">{t("Child work")}</span>
            <span className="text-muted-foreground">
              {children.active.length > 0
                ? `${children.active.length} ${t("active")}, ${children.done} ${t("done")}, ${children.cancelled} ${t("cancelled")}`
                : `${t("all")} ${children.total} ${t("terminal")} (${children.done} ${t("done")}, ${children.cancelled} ${t("cancelled")})`}
            </span>
          </div>
          {children.active.length > 0 ? (
            <div className="mt-2 flex flex-wrap gap-1.5">
              ...
              {children.active.length > 4 ? (
                <span className="rounded-md border border-border px-2 py-1 text-[11px] text-muted-foreground">
                  +{children.active.length - 4} {t("more")}
                </span>
              ) : null}
            </div>
          ) : null}
      ...
      {latestSilentRun?.outputSilence ? (
        ...
        >
          <p className="font-medium">
            {latestSilentRun.outputSilence.level === "critical"
              ? t("Stale-run watchdog alert")
              : t("Output silence watchdog warning")}
          </p>
          <p className="mt-1">
            {t("Latest active run has been silent for")}{" "}
            {formatSilenceAge(latestSilentRun.outputSilence.silenceAgeMs) ?? t("an extended period")}.
      ...
      {feedItems.length === 0 ? (
        <div className="rounded-md border border-dashed border-border px-3 py-3 text-sm text-muted-foreground">
          {renderActivityEvent
            ? t("Runs and activity will appear here once this issue has history.")
            : t("Historical runs without liveness metadata will appear here once linked to this issue.")}
        </div>
      ) : (
        <div className="space-y-1.5">
            ...
            return (
              ...
              >
                <div className="flex flex-wrap items-center gap-1.5">
                  ...
                  {(() => {
                    ...
                    const label = profile.applied === profile.requested
                      ? `${t("Profile:")} ${profile.requested}`
                      : profile.applied
                        ? `${t("Profile:")} ${profile.requested} → ${profile.applied}`
                        : `${t("Profile:")} ${profile.requested} (${t("unavailable")})`;
                    return (
                      <span
                        className={cn(
                          "rounded-md border px-1.5 py-0.5 text-[11px] font-medium",
                ...
                <div className="grid gap-2 text-xs text-muted-foreground sm:grid-cols-3">
                  ...
                  <div className="min-w-0">
                    <span className="text-foreground">{t("Last useful action")}</span>{" "}
                  ...
                  return (
                    <p className="min-w-0 break-words text-[11px] leading-5 text-amber-700 dark:text-amber-300">
                      {profile.requested === "cheap"
                        ? t("Cheap profile fell back to primary")
                        : `${profile.requested} ${t("profile unavailable")}`}
                      {": "}
                ...
                {run.nextAction ? (
                  <div className="min-w-0 rounded-md bg-accent/40 px-2 py-1.5 text-xs leading-5">
                    <span className="font-medium text-foreground">{t("Next action:")}{" "}</span>
          ...
          {feedItems.length > 20 ? (
            <div className="px-3 py-2 text-xs text-muted-foreground">
              {feedItems.length - 20} {t("older items not shown")}
            </div>
          ) : null}
        </div>
      )}
