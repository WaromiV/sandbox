  ...
  return (
    <div className="flex items-start gap-1.5 min-w-0 flex-1">
      ...
      <button
        ...
        onClick={handleCopy}
        title={copied ? t("Copied!") : t("Click to copy")}
      >
        {value}
      </button>
...
function RemovableIssueReferencePill({
  ...
  const removeLabel = `${t("Remove")} ${issueLabel} ${t("as blocker")}`;
    ...
    <>
      ...
      <Dialog open={isConfirmOpen} onOpenChange={setIsConfirmOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{t("Remove blocker?")}</DialogTitle>
            <DialogDescription>
              {t("Remove")} {confirmLabel} {t("as a blocker for this issue.")}
            </DialogDescription>
          </DialogHeader>
          <DialogFooter>
            <DialogClose asChild>
...
export function IssueProperties({
  ...
  const assigneeOptionsTrigger = (() => {
    if (assigneeOverrideLane === "cheap") {
      return <span className="text-sm">{t("Cheap model")}</span>;
    }
    if (assigneeOverrideLane === "custom") {
      ...
      return (
        <span className="min-w-0 text-sm break-words">
          {details.length > 0 ? `${t("Custom")} · ${details.join(" · ")}` : t("Custom adapter options")}
        </span>
      );
    }
    return <span className="text-sm text-muted-foreground">{t("Primary model")}</span>;
  })();
  const assigneeOptionsContent = supportsAssigneeOverrides ? (
    <div className="w-full space-y-3 p-2">
      <div className="space-y-1.5">
        <div className="text-xs text-muted-foreground">{t("Model lane")}</div>
        <div className="flex w-full overflow-hidden rounded-md border border-border" role="radiogroup" aria-label={t("Model lane")}>
        ...
        {assigneeOverrideLane === "cheap" ? (
          <p className="text-[11px] text-muted-foreground">
            {t("Sends")} <code>modelProfile: "cheap"</code>{" "}
            {assigneeCheapProfile?.adapterConfig && typeof (assigneeCheapProfile.adapterConfig as Record<string, unknown>).model === "string"
              ? <>{t("· adapter default")} <code>{String((assigneeCheapProfile.adapterConfig as Record<string, unknown>).model)}</code></>
              : assigneeCheapProfile
                ? <>{t("· uses the agent's configured cheap profile")}</>
                : <>{t("· falls back to the primary model if no cheap profile is configured")}</>}
          </p>
        ) : null}
      </div>
    ...
    <div className="w-full space-y-2 p-2">
      <p className="text-xs text-muted-foreground">
        {assignee
          ? t("This assignee's adapter does not expose editable issue overrides.")
          : t("Select a compatible agent assignee to edit these overrides.")}
      </p>
      <button
        type="button"
  ...
  const runExecutionButton = (stageType: "review" | "approval") => (
    <PropertyRow label="">
      ...
      >
        {stageType === "review" ? t("Run review now") : t("Run approval now")}
      </button>
    </PropertyRow>
  );
  const currentExecutionLabel = (() => {
    ...
    if (issue.executionState.status === "changes_requested") {
      return `${stageLabel} ${t("requested changes")}${participantLabel ? ` ${t("by")} ${participantLabel}` : ""}`;
    }
    return `${stageLabel} ${t("pending")}${participantLabel ? ` ${t("with")} ${participantLabel}` : ""}`;
  })();
  useEffect(() => {
  ...
  const currentMonitorLabel = (() => {
    if (issue.executionPolicy?.monitor?.nextCheckAt) {
      return `${t("Next check")} ${formatDate(new Date(issue.executionPolicy.monitor.nextCheckAt))}`;
    }
    if (issue.executionState?.monitor?.status === "cleared") {
      return t("Cleared");
    }
    if (issue.monitorLastTriggeredAt) {
      return `${t("Last triggered")} ${timeAgo(issue.monitorLastTriggeredAt)}`;
    }
  ...
  const monitorTrigger = (
    <span className="inline-flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-0.5">
      ...
      >
        {monitorNextCheckAt ? `${t("Next check")} ${formatMonitorOffset(monitorNextCheckAt)}` : currentMonitorLabel}
      </span>
      {monitorNextCheckAt ? (
  ...
  const monitorAttemptBadge = issue.monitorAttemptCount && issue.monitorAttemptCount > 0 ? (
    <span className="text-xs text-muted-foreground">
      {t("Attempt")} {issue.monitorAttemptCount}
    </span>
  ) : null;
  ...
  const scheduledRetryRelativeLabel = (() => {
    if (!scheduledRetryRelative) return t("Pending schedule");
    const action = scheduledRetryIsContinuation ? t("Continuation") : t("Retry");
    if (scheduledRetryRelative === "now") return `${action} ${t("due now")}`;
  ...
  const scheduledRetryAttemptBadge = scheduledRetryAttempt !== null ? (
    <span className="text-xs text-muted-foreground">{t("Attempt")} {scheduledRetryAttempt}</span>
  ) : null;
    ...
    <div className="flex w-full flex-col gap-2 p-2 text-xs">
      <div className="flex items-center justify-between">
        <span className="text-sm font-medium text-foreground">
          {scheduledRetryIsContinuation ? t("Scheduled continuation") : t("Scheduled retry")}
        </span>
      ...
      <dl className="grid grid-cols-[6rem_1fr] gap-y-1">
        ...
        {scheduledRetry.retryOfRunId ? (
          <>
            <dt className="text-muted-foreground">{t("Replaces run")}</dt>
            <dd className="text-foreground">
              <Link
      ...
      <div className="flex items-center justify-between gap-2">
        ...
        >
          {retryNow.isPending ? (
            ...
            <span className="inline-flex items-center gap-1.5">
              ...
              {retryNow.data?.outcome === "already_promoted" ? t("Already promoted") : t("Promoted")}
            </span>
          ) : (
            <span className="inline-flex items-center gap-1.5">
        ...
        <span className="text-right text-xs text-muted-foreground">
          {retryNow.isPending
            ? t("Promoting scheduled retry")
            : scheduledRetryRetryNowSuccess
              ? retryNow.data?.outcome === "already_promoted"
                ? t("Already promoted — run starting")
                : t("Promoted — run starting")
              : scheduledRetryIsContinuation
                ? t("Pulls continuation forward immediately")
                : t("Pulls retry forward immediately")}
        </span>
      </div>
    </div>
  ) : null;
    ...
    <div className="flex w-full flex-col gap-2">
      <div className="flex flex-col gap-2 md:flex-row">
        ...
        <input
          ...
          placeholder={t("What should the agent re-check?")}
      ...
      <div className="flex flex-col gap-2 md:flex-row">
        <input
          type="text"
          className="min-w-0 flex-1 rounded-md border border-border bg-transparent px-2 py-1 text-xs"
          placeholder={t("External service")}
  ...
  const assigneePickerOptions = orderItemsBySelectedAndRecent(
    [
      { id: "", kind: "none" as const, label: t("No assignee"), searchText: "" },
      ...(currentUserId
        ? [{
      ...
      ...(issue.createdByUserId && issue.createdByUserId !== currentUserId
        ? [{
            ...
            label: creatorUserLabel ? `${t("Assign to")} ${creatorUserLabel}` : t("Assign to requester"),
  ...
  const executionParticipantsContent = (
    ...
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={stageType === "review" ? t("Search reviewers...") : t("Search approvers...")}
        value={search}
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        <button
          ...
          onClick={onClear}
        >
          {stageType === "review" ? t("No reviewers") : t("No approvers")}
        </button>
        {currentUserId && (
          <button
            className={cn(
  ...
  const projectPickerOptions = orderItemsBySelectedAndRecent(
    [
      { id: "", kind: "none" as const, name: t("No project"), color: null as string | null },
  ...
  const parentTrigger = issue.parentId ? (
    ...
    <span className="text-sm text-muted-foreground">{t("No parent")}</span>
  );
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
          ...
          <PropertyPicker
            ...
            extra={
              <button
                ...
                className="inline-flex items-center justify-center h-5 w-5 rounded hover:bg-accent/50 transition-colors text-muted-foreground hover:text-foreground"
                onClick={() => updateAssigneeAdapterOverrides(null)}
                aria-label={t("Clear adapter options")}
                title={t("Clear adapter options")}
              >
                <X className="h-3 w-3" />
              </button>
            }
        ...
        {inline ? (
          <div>
            <PropertyRow label={t("Blocked by")}>
        ...
        <PropertyRow label={t("Sub-issues")}>
        ...
        {relatedTasks.length > 0 ? (
          <PropertyRow label={t("Related Tasks")}>
            <div className="flex flex-wrap gap-1">
