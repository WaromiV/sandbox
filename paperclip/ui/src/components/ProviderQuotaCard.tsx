        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            ...
            <CardDescription className="text-xs mt-0.5">
              <span className="font-mono">{formatTokens(totalInputTokens)}</span> {t("in")}
              {" · "}
              <span className="font-mono">{formatTokens(totalOutputTokens)}</span> {t("out")}
        ...
        {hasBudget && (
          <div className="space-y-3">
            ...
            <QuotaBar
              ...
              rightLabel={`~${formatCents(Math.round(weeklyBudgetShare))}${t(" / wk")}`}
          ...
          <>
            ...
            <div className="space-y-2">
              ...
              <p className="text-xs text-muted-foreground">
                ...
                <span className="font-mono text-foreground">{formatTokens(totalSubInputTokens)}</span> {t("in")}
                {" · "}
                <span className="font-mono text-foreground">{formatTokens(totalSubOutputTokens)}</span> {t("out")}
              </p>
              {subSharePct > 0 && (
                <>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {Math.round(subSharePct)}{t("% of token usage via subscription")}
                  </p>
                </>
              )}
            </div>
                  ...
                  <div key={`${row.provider}:${row.model}`} className="space-y-1.5">
                    ...
                    <div className="relative h-2 w-full border border-border overflow-hidden">
                      <div
                        ...
                        title={`${Math.round(tokenPct)}${t("% of provider tokens")}`}
                      />
                      ...
                      <div
                        ...
                        style={{ width: `${costPct}%` }}
                        title={`${Math.round(costPct)}${t("% of provider cost")}`}
                      />
                    </div>
                  </div>
                );
                        ...
                        <div key={qw.label} className="space-y-1">
                          <div className="flex items-center justify-between gap-2 text-xs">
                            ...
                            {qw.valueLabel != null ? (
                              ...
                              <span className="font-medium tabular-nums">{qw.usedPercent}{t("% used")}</span>
                            ) : null}
                          </div>
                          ...
                          {qw.detail ? (
                            ...
                            <p className="text-xs text-muted-foreground">
                              {t("resets")} {new Date(qw.resetsAt).toLocaleDateString(undefined, { month: "short", day: "numeric" })}
                            </p>
                          ) : null}
                        </div>
                      );
