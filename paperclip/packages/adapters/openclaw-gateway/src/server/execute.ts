...
function buildWakeText(
  ...
  options: {
    ...
    paperclipSkillRelPath?: string;
  },
): string {
  const claimedApiKeyPath = options.claimedApiKeyPath;
  const skillRelPath = options.paperclipSkillRelPath ?? "skills/paperclip/SKILL.md";
  const orderedKeys = [
    "PAPERCLIP_RUN_ID",
    "PAPERCLIP_AGENT_ID",
  ...
  const lines = [
    ...
    "",
    `Your bridge-managed paperclip skill is at \`${skillRelPath}\` inside your agent workspace — the openclaw bootstrap loads it for you. Follow its instructions verbatim for every paperclip API call.`,
    "",
    "Run this procedure now. Do not guess undocumented endpoints and do not ask for additional heartbeat docs.",
    "",
    "Set these values in your run context:",
    ...envLines,
    `PAPERCLIP_API_KEY=<JSON .token field from ${claimedApiKeyPath}>`,
    "",
    `Read ${claimedApiKeyPath} and use its \`.token\` field as PAPERCLIP_API_KEY. The file is JSON: { token, agentId, companyId, apiUrl, issuedAt }. The openclaw-bridge writes/rotates this token automatically — never edit it.`,
    "",
