...
const EDITOR_WORKSPACE_STORAGE_KEY = "openclaw:editor:workspaceId";

function loadStoredEditorWorkspace(): string | null {
  const storage = getSafeLocalStorage();
  if (!storage) return null;
  try {
    const value = storage.getItem(EDITOR_WORKSPACE_STORAGE_KEY);
...
function storeEditorWorkspace(id: string) {
  const storage = getSafeLocalStorage();
  if (!storage) return;
  try {
    storage.setItem(EDITOR_WORKSPACE_STORAGE_KEY, id);
  } catch {
...
export class OpenClawApp extends LitElement {
  ...
  @state() editorWorkspaceId: string | null = loadStoredEditorWorkspace();
  @state() editorFilePath: string | null = null;
  ...
  setEditorWorkspace(next: string) {
    if (this.editorWorkspaceId === next) return;
    this.editorWorkspaceId = next;
    this.editorFilePath = null;
    storeEditorWorkspace(next);
  }

  setEditorTarget(next: { workspaceId: string | null; filePath: string | null }) {
    const nextWorkspaceId = next.workspaceId ?? this.editorWorkspaceId;
    if (nextWorkspaceId && nextWorkspaceId !== this.editorWorkspaceId) {
      this.editorWorkspaceId = nextWorkspaceId;
      storeEditorWorkspace(nextWorkspaceId);
    }
    this.editorFilePath = next.filePath ?? null;
  }
